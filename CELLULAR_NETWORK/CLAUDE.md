...
- **`BOILER/dashboard/public/js/cellular.js`** (`cellularOnTabShow()` entry, called by network.js's `showTab`). Leaflet map: home pin + 2 km circle + one **carrier-colored** `circleMarker` per antenna (Cellcom `#8e44ad` purple / Pelephone `#2980b9` blue / PHI-HOT+Partner `#e74c3c` red / other grey) with a hover tooltip (carrier / address / technology / radiation % colored by level / distance). Below the map: a **sortable list** (click Carrier / Radiation / Distance headers) — each row click pans+zooms the map to that antenna and opens its tooltip; a 📄 link opens the operating/construction permit PDF. **Map controls** (2026-06-13): **🔍 Zoom to nearest** (`cellZoomNearest(5)` — fits the 5 closest antennas + home then +1 zoom step) + **⤢ Fit all** (`cellZoomAll()` — fits the whole radius circle). Summary strip shows count + per-carrier legend + last-ingest time. `invalidateSize()` on tab show (map div starts `display:none`). Leaflet 1.9.4 loaded in `network.html` `<head>`; cache-bust `cellular.js?v=6`, `network.js?v=11`.

**Estimated influence at home** (2026-06-13). The registry's radiation number is measured at each tower's OWN worst-case point, **not at the apartment** — so true exposure at home is NOT derivable from this data (needs an RF meter or a full propagation model with EIRP / azimuth / tilt / buildings). What's shown is an explicitly-labelled **relative estimate** via the inverse-square law: `influence ∝ strength / distance²`, where `strength = max_theoretical_uw_per_cm2` (µW/cm², present on 72/76; falls back to `max_measured_pct_of_threshold`). `_cellComputeInfluence()` normalises to a per-tower **share %** (Σ = 100 %, "this tower = X % of the local RF") + a 0..1 norm vs the strongest. Surfaced three ways: (1) an **Est. influence** column in the list (bar + share %, the default sort key, desc) + the same value in each map tooltip; (2) a **summary line** naming the dominant tower + per-carrier share (live: Cellcom 42 % · PHI 38 % · Pelephone 19 %, dominant Cellcom @274 m ≈ 13.8 %) with a bold **"⚠ Estimate only — not a measurement"** caveat; (3) a toggleable **🔥 Influence heatmap** (`leaflet.heat` 0.2.0, loaded in `<head>`) weighting each tower by `sqrt(norm)` so near+strong towers glow hottest — shows which direction the strongest RF comes from. All client-side in `cellular.js` (no server/ingest change); heatmap guarded by `typeof L.heatLayer === 'function'`.

**Phone live signal — personal-exposure proxy** (2026-06-13). The honest insight from the influence discussion: the dominant RF at home is usually **your own phone's uplink TX**, which RISES when the tower signal is weak (adaptive power control), and the phone is centimetres from you. So a live "how hard is my phone transmitting" readout is a better personal-exposure indicator than the tower map. Endpoint **`GET /api/cellular/phone-signal`** (in `routes-cellular.js`, gets `getHaToken` from server.js) reads the **HA Companion-app sensors** for the tracked phone (Galaxy Z Fold5, entity prefix `sm_f946b` — `PHONE_PREFIXES` const). It **auto-discovers** the cellular-signal entity by pattern (`sensor.*_signal_strength|*cellular*` minus wifi) so it works regardless of exact naming and lights up the moment the sensor is enabled; also returns network type, Wi-Fi dBm/connection, battery. `_signalQuality(dbm)` maps LTE dBm → quality + implied TX effort + exposure (≥−80 excellent/low … <−110 very-poor/high). Frontend card `#cell-phone` (polls every 20 s) shows signal + transmit-effort + est. personal exposure + Wi-Fi context, or — until enabled — an inline how-to. **One-time user step:** enable **“Cellular signal strength”** in the HA Companion app on the phone (Settings → Companion app → Manage sensors); Android exposes it, ships it DISABLED. cache-bust `cellular.js?v=7`.
