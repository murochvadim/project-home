...
- **Move in Corridor also stands down while leaving (2026-06-17):** the rule's `suppress_monitoring` is now `(transit_mode == 'Corridor_From_Home') OR away_countdown_active`, where `away_countdown_active = state.shared['daily_welcome.suppress_until_ts'] > now`. This skips the **whole** monitor / awtrix / FR / pixoo chain (not just the Pixoo, which the lock already blocks) while an away countdown owns the screen — catching the common leaving case where the transit classifier is still at `Corridor_Visit_Home` at rising-edge time (it upgrades to `From_Home` a few seconds later), so the delayed **FR `start_recognition`** no longer fires while you walk out. Only the always bucket (corridor light) fires. On arrival there's no away countdown → the welcome + FR fire normally.
