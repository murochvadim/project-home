...
// Reminders — project-wide due-reminders badge (pills + measure schedules now);
...
require('./routes-car-snapshot')(app, () => mqttClient);
...
async function runHealthChecks() {
  ...
  // RP03 CPU temperature — same shape as RP01/RP02 (rp03_project user; skipped when RP03
  // monitoring is paused). Const-arrow so the architecture-guard hook allows it.
  const rp03TempRead = async () => {
    if (mon.rp03 === false) return { temp_c: null };
    const ssh = new NodeSSH();
    try {
      await ssh.connect({ host: '192.168.1.169', username: 'rp03_project', privateKeyPath: SSH_KEY, readyTimeout: SSH_TIMEOUT });
      const out = (await ssh.execCommand('cat /sys/class/thermal/thermal_zone0/temp')).stdout.trim();
      ssh.dispose();
      const t = parseInt(out, 10);
      return { temp_c: Number.isFinite(t) ? Math.round(t / 1000) : null };
...
const DBV_GROUPS = [
  ...
  ['Privacy',               ['privacy_sites','privacy_site_docs','privacy_site_receipts','privacy_doc_crypto','privacy_sheets','visited_places','journal_entries','journal_media']],
  ...
  ['Kitchen',               ['kitchen_categories','kitchen_recipe_categories','kitchen_products','kitchen_shopping_lists','kitchen_shopping_items','kitchen_settings']],
...
app.get('/api/health/db-volumes', async (req, res) => {
  try {
    ...
    const tsCol = {
      ...
      esp_boards: 'created_at', jura_daily: 'updated_at', jura_drinks: 'ts',
      ...
      kitchen_categories: 'updated_at', kitchen_recipe_categories: 'updated_at', kitchen_products: 'updated_at', kitchen_shopping_lists: 'created_at', kitchen_shopping_items: 'added_at', kitchen_settings: 'updated_at',
