...
- ESP32-C3: `C:\Users\muroc\Arduino_Projects\My_Bathroom_Smell_Claude\` (minimal Mosquitto-only example with AUTO-cycle pump state machine; sketch + `device_id` constant + DB `esp_boards.id` + MQTT topic prefix all renamed from `My_Bathroom_Smell_6` → `My_Bathroom_Smell_Claude` on 2026-05-23)
