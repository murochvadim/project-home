...
(function () {
  ...
  async function hpInit() {
    ...
    _hpMqtt = mqtt.connect(`ws://${HP_BROKER_HOST}:${HP_BROKER_PORT}`, {
      ...
      clientId: 'hasp-my-bathroom-tab-' + Math.random().toString(36).slice(2, 10),
  ...
  // ════════════════════════════════════════════════════════════════════════
  // ─── My BathRoom Smart Switch (Tuya TS0044) — bindings UI ────────────────
  // 4 buttons × 2 events (single, hold) = 8 binding slots, multi-device per
  // slot. Storage: dashboard_settings.my-bathroom.smart_switch_bindings. Mirrors
  // the HASP-button bindings pattern above with sw* prefix instead of bc*.
  // ════════════════════════════════════════════════════════════════════════
  const SW_STORAGE_KEY = 'my-bathroom.smart_switch_bindings';
