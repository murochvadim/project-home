...
- Start a candidate event when two consecutive rows differ by ≥ 0.5°C (descent trigger). Extension: keep chaining forward as long as the next row is within `JITTER_TOL_C` (0.2°C) of the running minimum — the event ends when temperature recovers meaningfully above that floor (fixed 2026-04-24: old per-step ≥ 0.3°C threshold was sized for 5-min cron rows and bailed almost immediately once WF96C MQTT ingest at 2-3 s cadence replaced it on 2026-04-23, silently fragmenting real drops into tiny sub-events that fell below `consumption_temp_delta` and never landed in `boiler_consumptions`). Only record completed events (drop has ended)
