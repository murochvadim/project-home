...
- **Cards**: (A) **Current Period (Water)** — meter-ready placeholder; renders "meter not connected yet" until a flow sensor is added (Phase 2). (B) **Water Billing Cycle** (start_day / length_months default 2 = bi-monthly / current_period_start_date). (C) **Water Tariff** — full Israeli two-tier model: persons_in_household, low-tier ₪/m³ + quota (base m³/household/month + per-extra-person m³/month), high-tier ₪/m³, sewage_mode (bundled|separate) + sewage rate, fixed charge, VAT, currency. ⚠ defaults are PLACEHOLDERS flagged "verify against your bill" (Israeli rates/quotas vary by corporation + period). (D) **Past Water Bills** — `+ Paste bill (PDF)` or `+ Add manually` → **editable confirm modal** (period / **private m³** / **shared m³** / actual ₪) → `POST /api/water/bills`; table **Period / Private m³ / Shared m³ / Estimate / Actual / Diff** (the Source + Uploaded columns were dropped per user 2026-06-08). `total_m3 = private + shared` (computed server-side, drives the estimate). `water_bills` has `private_m3` + `shared_m3` columns.
