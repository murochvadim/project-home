...
- **Cards**: (A) **Current Period (Water)** — meter-ready placeholder; renders "meter not connected yet" until a flow sensor is added (Phase 2). (B) **Water Billing Cycle** (start_day / length_months default 2 = bi-monthly / current_period_start_date). (C) **Water Tariff** — full Israeli two-tier model: persons_in_household, low-tier ₪/m³ + quota (base m³/household/month + per-extra-person m³/month), high-tier ₪/m³, sewage_mode (bundled|separate) + sewage rate, fixed charge, VAT, currency. ⚠ defaults are PLACEHOLDERS flagged "verify against your bill" (Israeli rates/quotas vary by corporation + period). (D) **Past Water Bills** — `+ Paste bill (PDF)` or `+ Add manually` → **editable confirm modal** (period / m³ / actual ₪) → `POST /api/water/bills`; table Period / m³ / Estimate / Actual / Diff / Source / Uploaded.
...
- **PDF parser** `_parseWaterBill(text)` — tuned to the **מי הוד השרון (Mei Hod HaSharon)** bill. Grabs each field by its Hebrew label, picking the **closest** `NNN.NN` (either side, units like `(מ"ק)`/`(כולל מע"מ)` consumed into the label) so pdf-parse's jumbled RTL order doesn't matter. Extracts: period (`MM-MM/YYYY` → start/end), total m³ (`סה"כ לחיוב`), total ₪ (`סה"כ לתשלום`), VAT% (from `סה"כ ללא מע"מ` + total), persons (`נפשות`). **Upload only PARSES + RETURNS** (no insert) — the confirm modal lets the user verify/fix before saving, so an imperfect parse is still usable. Other corporations (Hagihon/Mei Avivim) need their own anchors added. Validated against a sample bill: period 07-08/2021 → m³ 18.95, ₪144.85, VAT 17%, 3 persons.
