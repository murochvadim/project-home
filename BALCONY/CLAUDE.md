...
| `balcony.smart_switch_bindings` | JSONB | TS0044 4-button scene-remote bindings (see Balcony Smart Switch tab). |
...
- **somfy + fan** (tab renamed from `Somfy` 2026-07-23) — the same `balcony_bridge` board's CC1101 also drives a **433.92 MHz swing fan** (fixed-code RF, rc-switch **proto 6, 12-bit**). Firmware **`balcony_bridge_devices` (`v23-devices`)** — unified BoBo + Somfy + Fan. **Fan card** (in `balcony.html`, below the Somfy card): `⏻ Fan On` / `⭘ Fan Off` / `Speed +` / `Speed −` / `💡 Light` + a `fan: ON/OFF` chip + `light:` chip + `speed: N` label. Buttons call `fanCmd(<code>)` → `POST /api/esp/boards/balcony_bridge/command` `fan_tx:<code>` (the `fan_tx` action is in the board schema); the board TXes the code via CC1101 (retune → send 8× → back to RX). **Verified true codes (2026-07-23):** On **351** · Off **381** · Speed+ **349** · Speed− **375** · Light **382**. **⚠ RX mirroring of the PHYSICAL remote is NOT viable and is DISABLED** — rc-switch decodes this remote inconsistently: the SAME button decodes to a *different* integer each boot (On came out as 351 / 367 / 381, the last colliding with Off). Chasing the number is hopeless, so v23 removed the `RF[raw]` diagnostic + the auto-mirroring; **the dashboard is the source of truth** for the `fan:`/`light:`/`speed:` estimate (updated from the TX command via `applyFanCode(..,"tx")`, which the board publishes in `/status` as `fan_power`/`fan_light`/`fan_speed`; `js/balcony.js` `sfPoll` renders the chips). TX works because a proto-6 *encode* of the captured codes reproduces a close-enough waveform the fan accepts; a proper fix for remote-mirroring would need raw-waveform capture/replay (not built). Fan state is a client/estimate only (no real feedback). Codes live inline in `balcony.html` `onclick` + the firmware `FAN_*` `#define`s — no DB, no `routes-*.js`, no `server.js` change. Firmware not in git (bakes creds) — sketch at `C:\Users\muroc\Arduino_Projects\balcony_bridge_devices\`.
