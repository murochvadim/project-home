...
| Key | Shape | Purpose |
|-----|-------|---------|
| `balcony.somfy_motors` | `[{idx,name,invert}]` (4) | Somfy tab — the 4 motor names (Left/Right Roof, Left/Right Curtain) + per-motor open/close-invert. Firmware indexes/addresses are fixed, so this only holds display config. |
