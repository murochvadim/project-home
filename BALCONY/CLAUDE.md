...
- **Star Projector** (added 2026-05-26) — control surface for the Tuya WIFI Star Projector 2 (id `bfecd037d10a0ffcfagj8e`, IP `192.168.1.185`, MAC `00:33:7a:3b:bd:ed`, room `Balcony`, `protocol='local'`, `version='3.5'`). Five cards: Power (On/Off — DPS 20 bool), Mode (white / colour / scene / music — DPS 21 enum), Brightness (range 10..1000 — DPS 22, sent on slider release with a 3 s touch-debounce so live-poll repaint doesn't fight the user mid-drag), Colour (browser color-picker → Tuya HSV 12-hex string `HHHHSSSSVVVV` H 0..360 / S 0..1000 / V 0..1000 — DPS 24, auto-switches mode to `colour`), Scene (8 preset buttons — DPS 25 ids `1..8`, auto-switches mode to `scene`). All controls write through a single new endpoint `POST /api/devices/:id/dps` (body `{dps: {key: value, ...}}`) — see "Direct Tuya local DPS write" below. 5 s state poll; optimistic repaint after each send so the UI feels instant. Cache-bust: `balcony.js?v=20260526c`. Tab title plus header dot show online status from `last_seen` freshness (< 10 min = green).
