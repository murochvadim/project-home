...
## Built — tablet alerts from Notifications (2026-07-26)
The tablet can now **flash an alert** driven by the **Main Agent → Notifications** system (e.g. *presence in the corridor → big blinking red Door + beep*).
- **Trigger:** the Notifications rule gained a generic **`device_presence`** trigger (`trigger_param` = a presence/motion/door sensor's device_id) — fires on the sensor's **rising edge** (`dps['1']` none/false→present). ⚠ Corridor Presence **double-reports** (`tcp_push 'presence'` + `ha_api true`); handled by the multi-value check `in ('presence',True,'true',1)` + rising-edge dedup + `throttle_min` → **one** alert. Primed on `_first_eval` so a reload never spuriously fires.
- **Surface:** a notification's `surfaces.tablet` = `{enabled, icon, color, blink, sound, duration_sec}` (JSONB, **no migration**). When it fires the rule emits `{protocol:'panel_alert', …}`; the engine's **new `_dispatch_command` branch** (`protocol=='panel_alert'`) `publish_raw`s it to **`mur/home/device/panel/alert`** *(engine-core change → one-time rule-engine restart)*.
- **ACL (LXC 107):** `rule_engine` += `write mur/home/device/+/alert`; `dashboard_browser` += `read mur/home/device/+/alert` (2 lines).
- **Tablet (`panel.js`/`panel.css`):** subscribes to `panel/alert`; renders a full-screen **blinking icon (from the SVG set, in the alert colour) + message**, auto-dismiss after `duration_sec` or tap-to-dismiss, and a **synthesized Web-Audio beep** loop. ⚠ **Audio autoplay:** unlocks on the first screen touch (kiosk gets tapped); the very first alert on a freshly-loaded, untouched panel is silent — enable **Fully Kiosk autoplay** for guaranteed cold-start sound.
- **Editor (`notifications-tab.js` + `main-agent.html`):** the trigger dropdown gains **"Presence detected (a sensor)"** + a sensor selector; a **"📟 Also flash the Balcony tablet"** box (icon / colour / blink / sound / duration). Stored in `surfaces.tablet`.
- **Verified end-to-end** (2026-07-26) via a sandbox def + a fake presence device (no real corridor automation, panel config untouched): rule fired → `panel_alert (icon=door dur=8s)` → the tablet (dashboard_browser) received the exact payload.
