...
- **Per-tile placement (2026-07-26):** each tile carries **`row` + `pos`** (page = which page it lives on). The editor has a **Page** dropdown (move a tile to another page) + **Row** + **Position** number inputs per tile, and sorts tiles by `(row, pos)`. The PWA renders each configured **row** as a horizontal band of **fixed-width (215px) tiles**, ordered left-to-right by `pos` — **variable tiles per row** (user chose this over a fixed column grid), rows don't force column alignment. Backward-compatible: tiles missing `row/pos` default to row 1 / array order.
