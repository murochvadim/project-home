...
| `tuya.py` | Local Tuya TCP per-device threads + gateway Zigbee hub. Watchdog respawns silent threads (since 2026-05-15). **UDP-broadcast IP rediscovery (since 2026-05-27):** `tinytuya.find_device(dev_id)` runs at the start of every reconnect iteration after the first (when `attempt > 0`). If the cached IP differs from the device's current broadcast-announced IP, the local variable updates AND the new IP is persisted to `devices.local_ip` via a short-lived psycopg2 connection. Self-heals from DHCP rotation within one reconnect cycle (~30 s) — no manual SQL, no restart needed. The `device_agent.py` startup query already used `COALESCE(n.ip, d.local_ip)` to pull ARP-observed IPs at boot; the new code closes the runtime gap so long-running processes (10+ days uptime) also benefit. Proven 2026-05-27 on `Aura Air switch` (.199 → .196) and `Gas sensor` (.118 → .107) — both detected + persisted live. Devices that don't broadcast (offline or non-stock firmware) get a `rediscovery timed out` INFO log and fall back to the cached IP. | active development |
