...
| `tuya_cloud.py` | Tuya Cloud API client. **IR-remote keepalive** for `device_type='remote'` (Tuya `wnykq` IR hubs). Original 2026-05-15 design (commit f72a773) bumped `last_seen` on any successful empty-DPS status response — but the cloud status API returns `success=True` even when the device is offline, so dead-battery remotes showed perma-alive (caught 2026-05-16, Maya Bedroom Remote falsely alive 10 h after battery death). **Current design (since 2026-05-16)** issues a secondary `/v1.0/devices/<id>` call and only bumps `last_seen` if `result.update_time` is within `REMOTE_UPDATE_TIME_STALE_SEC` (30 min). `update_time` is the actual last device → cloud check-in timestamp — `online` flag from the same API is unreliable (observed `online: True` on the dead-battery device). Helper: `_remote_recently_active()`. Scoped to `device_type='remote'`; other cloud-poll devices unaffected. | active development |
