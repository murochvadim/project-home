...
## HA `valve` domain support (2026-07-31)

`ha_api.py` gained the HA **`valve`** domain for the **HCT-636 wireless water timer** (2 zones = `valve.water_timer_valve_1` / `_2`, Tuya Sub-GHz behind the HCG-003 gateway, exposed by the HA Tuya cloud integration). Two changes: (1) the entities are registered in `HA_DIRECT_DEVICES` (`'valve.water_timer_valve_N': [('valve.water_timer_valve_N', 'state')]`) so their state flows into `devices.last_state.state` (`open`/`closed`/`opening`/`closing`); (2) `_ha_state_to_dps_value` maps `domain in ('cover','valve')` → raw state (was `cover` only). Each zone is a `devices` row `protocol='valve'` `device_type='valve'` (room Balcony), state-only here — **open/close control is HA-mediated elsewhere** (dashboard `routes-valve.js` → `valve.open/close_valve`; rule engine `_dispatch_valve`). Verified: `HA map (ha_direct)` count +2, both zones ingest `closed` via `ha_api`. See root CLAUDE.md "Balcony Irrigation" + [[project_agent_water_valve]].

## LetPot hydroponic (HA-mediated, `protocol='letpot'`) (2026-07-31)

`ha_api.py` `HA_DIRECT_DEVICES` gained a multi-entity entry keyed **`lph_max_a687`** (LetPot LPH-Max, via the official HA **LetPot cloud** integration) mapping **12 entities** across 5 HA domains → dp keys: 4 `switch` (`power`/`pump_cycling`/`auto_mode`/`alarm_sound` → bool), 2 `number` (`light_brightness`/`plants_age`), 2 `select` (`light_mode`/`temp_unit`), 2 `time` (`light_on`/`light_off`), 2 `sensor` (`temperature`/`water_level` → float). **No `_ha_state_to_dps_value` change** — switch→bool, sensor→float, and number/select/time fall through the default `return state` (raw value/string), which is what the card needs. One `devices` row `id='lph_max_a687'` `protocol='letpot'` `device_type='hydroponic'` (room Balcony, `dps_labels='{}'` to keep the All-Devices grid one clean row). **Control is HA-mediated in the dashboard**, not here: `routes-letpot.js` `POST /api/letpot/set {entity_id,value}` → `switch.turn_on|off` / `number.set_value` / `select.select_option` / `time.set_value`. Surface = Balcony → Irrigation tab, LetPot Max card. See root CLAUDE.md "Irrigation tab" + [[project_agent_letpot]].
