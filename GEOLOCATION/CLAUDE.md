...
- **RULE 4 — Places far-teleport (2026-07-26, `clean_place_phantoms`, `PLACE_TELEPORT_PATH_RATIO`):** the FIRST janitor rule that touches the **Places layer** — Rules 1-3 act on `phone_trips` (Home-trips) ONLY, so `phone_place_trips`/`phone_places` (from `geo_places.py`) had **no phantom cleaner**. A GPS cache-replay that parks the phone at a far ghost long enough for `geo_places` to build an anchor makes a bogus place + legs — e.g. a **115 km `الجيزة, الأردن`** stay reached on a **12 m path** (2026-07-25, anchor #64). Signal (stored columns only, no ping re-fetch): a `home_to_place`/`place_to_home` leg with `max_dist_m > teleport_far_m` (20 km) **AND** `path_length_m < place_teleport_path_ratio` (**default 0.5**) `× max_dist_m` — a real far leg has path ≥ its straight-line distance, so this can ONLY fire on a teleport. The **phantom anchor** = that leg's place endpoint (`home_to_place`→`to_place_id`, `place_to_home`→`from_place_id`); delete it **+ EVERY leg referencing it** (in AND out). The closer real downstream anchor survives — it is never a home-teleport target. `--dry-run` supported; tunable `place_teleport_path_ratio` in `dashboard_settings.geolocation`. First live run 2026-07-26 removed anchor #64 (الجيزة, الأردن) + legs #159/#160, kept the real Ashdod anchor #65. Rules 1-3 + `geo_places.py`/`owntracks_ingest.py` untouched.
