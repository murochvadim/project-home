...
| Main Agent | `main-agent.html` | Rule engine state, rules table, room grid. **Base Rule Settings** tab (since 2026-04-22) — apartment-wide Layer 0/1 rule authoring in numbered containers of segment-based sentences (`dashboard_settings.apartment.rule_sentences`). Same UI pattern as Living Room Rule Settings: drag-reorder rules, `+Dev` chips with click-to-replace / ×-remove, Save All / Discard, `● Unsaved changes` badge + beforeunload warning, silent on-load migration of legacy `@Name:dps_key` tokens → `@Name dps_label`. Consumed by AI sentence→Python pipeline — see [RULES/TODO_RULES.md](RULES/TODO_RULES.md). **Tab-bar live display** (since 2026-04-26) — right-aligned strip showing `time mode` + `next sunrise` + `next sunset`, fed by `state.shared` from the `Home Time Periods` rule, refreshed on the existing 10 s poll. |
...
- Current rules: Home Time Periods, Mode Buttons, Home Activity, People Home, Boiler Consumption Classify, Wallmote Handler, Evening Lights, Daily_Wellcome (pixoo)
...
- **Evening Lights (action rule 2026-04-26):** sentence-driven turn-on rule. Two firing scenarios — (A) evening kick-in (`time_mode` just transitioned to `evening` AND `home_mode == 'home'`); (B) late arrival (`home_mode` just transitioned `away|abroad → home` AND current `time_mode` is in the active set). Active time modes are declared in `s_el2` (`Evening Lights: active time modes are evening, night, late_night`), device list in `s_el1` (`evening lights are @<Device> <Channel>, …`). **Latched per home period** — once fired, won't refire until `home_mode` leaves `home`, then comes back. Safe no-op when either sentence is missing. depends_on: `["Home Time Periods", "Mode Buttons"]`.
