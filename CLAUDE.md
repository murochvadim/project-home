...
| My BathRoom Agent | `my-bathroom.html` | Per-room agent for My BathRoom — touch panel, smart switch, smell pump, and bathroom-area devices. Dashboard-only agent (no LXC service). Tabs: **Panel** (placeholder for future OpenHASP panel mounted in My BathRoom) and **Smart Switch** (placeholder for future TS0044-class wireless scene remote). Storage: `dashboard_settings.my-bathroom.*` keys (none yet — populate as features land). Rules live under Main Agent → Base Rule Settings (no per-agent group, per the 2026-05-05 consolidation pattern). See [MY_BATHROOM/CLAUDE.md](MY_BATHROOM/CLAUDE.md). |
...
- **HASP + Awtrix as rule targets via the same dps_config alias pattern (since 2026-05-05):** the `hasp:balcony` and `awtrix_05ec2c` `devices` rows now declare controllable channels in `dps_config` exactly like ESP boards. Dispatch lives in the rule engine's HASP and Awtrix branches:
  ...
  - **HASP `backlight` channel** → `action_on='backlight_on'`, `action_off='backlight_off'` → publishes `hasp/balcony/command/backlight on|off`.
  - **HASP `page` channel** (`type='page_select'`, `min:1, max:12`) → `action_on='goto_page'` → publishes `hasp/balcony/command/page <N>` where `<N>` comes from `cmd['page_num']` carried on the binding/sentence chip. **Parameterized alias** — the only one in the project; the rule handlers (Wallmote, Smart Switch, Balcony Buttons) pass `page_num` through from binding to command, and the fallback for legacy `channel: null` bindings explicitly skips parameter-needing aliases when the cmd lacks the parameter so on/off bindings can't accidentally fall through to a `goto_page` route.
  ...
  - **Sentence chip parser** — [`RULES/_display_chips.py`](RULES/_display_chips.py) extended from display-only to display+panel. Recognizes `@<PanelName> on` / `off` (backlight) and `@<PanelName> Page <N>` (case-insensitive). All chip-using rules (currently `Evening Lights`, future ones via `/create-rule`) inherit panel chip handling automatically.

- **Smart Switch dupe-event cooldown (since 2026-05-05):** `balcony_smart_switch_handler.py` adds a 1 s per-slot cooldown because this MOES TS0044 firmware emits the same `dps['action']` value twice in rapid succession on every press. Without dedupe each binding fires twice; if the binding targets a device that doesn't tolerate two power-toggles 50 ms apart (e.g. Awtrix's `/power`), the second event cancels the first and the device appears unresponsive. Same pattern as the `balcony_buttons` HASP-press cooldown.
...
| My BathRoom Agent | `MY_BATHROOM/` | `MY_BATHROOM/CLAUDE.md` — dashboard-only per-room agent for My BathRoom. Panel + Smart Switch tabs (placeholders for future hardware). Smell pump (`My_Bathroom_Smell_6` ESP board) controllable via the existing dps_config alias pattern. No rule group — rules live under Main Agent → Base Rule Settings. |
