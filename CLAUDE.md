...
| Main Agent | `main-agent.html` | Rule engine state, rules table, room grid. **Base Rule Settings** tab (since 2026-04-22) — apartment-wide Layer 0/1 rule authoring in numbered containers of segment-based sentences (`dashboard_settings.apartment.rule_sentences`). Same UI pattern as Living Room Rule Settings: drag-reorder rules, `+Dev` chips with click-to-replace / ×-remove, Save All / Discard, `● Unsaved changes` badge + beforeunload warning, silent on-load migration of legacy `@Name:dps_key` tokens → `@Name dps_label`. Consumed by AI sentence→Python pipeline — see [RULES/TODO_RULES.md](RULES/TODO_RULES.md). |
