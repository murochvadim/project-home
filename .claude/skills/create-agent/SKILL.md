...
If **Remove**:
  ...
  7. Generate + run teardown SQL: `migrations/agent_<slug>_teardown.sql` with:
     ```sql
...
1. Write accumulated SQL to `migrations/agent_<slug>_audit_<yyyymmdd>.sql` (idempotent statements)
...
Full contents of `migrations/agent_<slug>_setup.sql` shown here — including agents INSERT, CREATE TABLE statements, retention_policies INSERTs, optional seed rows.
...
### 14a. Auto — DB writes
Generate `migrations/agent_<slug>_setup.sql` locally, then:
```bash
scp migrations/agent_<slug>_setup.sql root@192.168.1.227:/tmp/agent_<slug>_setup.sql
...
### SQL file (`migrations/agent_<slug>_setup.sql`)
```sql
...
**How to apply**: when features are added to this agent, place storage under the above namespace, rules under the above group, docs in the above location, SQL migrations under `migrations/agent_<slug>_*.sql`.
```
