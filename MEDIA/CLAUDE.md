...
**UI** (`media.html` + `media.js`, Player tab → QNAP Media grid): **☑ Select** files → the select bar shows **📁 Move to folder…** (opens a **navigable folder-picker modal** — breadcrumb + ⬆ Up + drill-into folders, "📁 Move here" targets the folder you're viewing, or type a new folder name; folders fetched fresh via `/api/media/browse`, not grid state), **📥 Download** (2026-08-09 — saves each selected file to the browser/laptop; each is a link to `GET :8766/api/media/stream/<rel>?dl=1`, and the `?dl=1` makes `stream_file()` set `Content-Disposition: attachment` server-side so the browser SAVES instead of playing — no blob/`createObjectURL`, so multi-GB videos don't blow up memory; multiple files staggered ~800 ms so the browser doesn't drop them. `downloadSelected()` in `media.js`; inline path unchanged when `?dl=1` is absent), and **🗑 Delete** (bulk file delete, confirms). Each **folder tile** has a **🗑** (top-right, like the ✏️ on file tiles) → deletes the folder + contents (confirms). Every delete path confirms (incl. `deleteCrop`, fixed same day). NOTE: `MEDIA_API = http://192.168.1.138:8766` (player_service, sends `Access-Control-Allow-Origin: *`); the dashboard calls it cross-origin.
...
#### Analyzer Agent tab
- **Auto Run Status** card: pending/processing/ready/error counts, progress bar, last run. **Red/green `#az-auto-badge` pill** next to the title (2026-08-05) = the face analyzer's auto mode (`analyzer_settings.auto_enabled`): **ENABLED** green / **DISABLED** red / — grey; refreshed on tab-load, on Media-Settings load, and immediately when Auto Mode is saved (`setAzAutoBadge` in media.js). **Both card titles carry a plain-language hover tooltip** (`title=` + `cursor:help`) explaining what the card does. ⚠ The "Overall" numbers are a **live readout of `media_library.status`** — NOT settings; there's nothing to "set to 0" there (to stop the analyzer, set Auto Mode → Disabled).
