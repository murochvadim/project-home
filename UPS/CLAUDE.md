...
| [scripts/doshutdown](../scripts/doshutdown) → `/etc/apcupsd/doshutdown` (PVE) | The orchestrator. SAFETY_MODE-gated. **Phased shutdown** (added 2026-04-30): [1] QNAP halt via CGI → [2] **Phase A clients sequential, 60 s each** (LXCs 100, 103, 104, 105, 106, 108, 109, 110, 111 + VM 101, halted one-by-one in numerical order — NOT parallel; the code blocks per guest) — these all write to Postgres / MQTT, must finish first → wait → [3] **Phase B services sequential, 30 s each** (LXCs 102 Postgres + 107 MQTT) — safe to halt now, no clients writing → wait → [4] write Phase 4 marker `/etc/apcupsd/last_shutdown_reason` → `shutdown -h now`. The phasing prevents transactions getting rolled back when Postgres/MQTT die mid-write — each client gets time to flush before its broker dies. |
...
- **Phase 4 Part B — LIVE** (confirmed live 2026-07-04): the orchestrator is armed — `SAFETY_MODE` **absent**, `BATTERYLEVEL=30`, `MINUTES=15` (note: 15, not the originally-planned 8), `RECOVER_AUTO=yes`, `apcupsd` active+enabled, `apcupsd-auto-recover.service` enabled. On a real BATTERYLEVEL/MINUTES trip the orchestrator now shuts the stack down for real and auto-recovers on mains-restore. (The BATTERYLEVEL=95 mains-pull end-to-end test / KILLPOWER + auto-boot verification was not re-run in this session — treat that specific validation as unconfirmed here.)
- **2026-07-04 LXC 110 (Email agent) added to UPS shutdown.** `110:lxc` appended to `PHASE_A_CLIENTS` in `doshutdown` + `doshutdown_rehearse` (it writes to Postgres/MQTT → a Phase-A client, must flush before 102/107 die in Phase B; Phase A fully completes before Phase B starts, so its DB writes are guaranteed flushed). **Recovery needed no change** — `doshutdown_recover` already started `110:Email` in the QNAP-independent batch, and `email-agent.service` is `Restart=always` so it self-heals if it starts before Postgres/MQTT are ready. Deployed to `/etc/apcupsd/` on PVE + verified (`bash -n`, deployed arrays show 110). Every guest is `onboot=0` (all-or-nothing recovery model — the CLAUDE.md "onboot=1" note from the 2026-04-30 hardening is stale; the current live state is onboot=0 across all guests).
- **2026-08-11 LXC 111 (Robot / TonyBot) added to UPS shutdown + recovery.** `111:lxc` appended to `PHASE_A_CLIENTS` in `doshutdown` + `doshutdown_rehearse` (it writes vision/camera state to Postgres/MQTT → a Phase-A client, must flush before 102/107 die in Phase B), and `"111:Robot"` added to the QNAP-independent recovery batch in `doshutdown_recover` (no QNAP/NFS dependency — go2rtc + vision venv are local to the LXC). **`pct set 111 --onboot 0`** applied — 111 was provisioned `onboot=1`, which violated the all-or-nothing recovery contract (`feedback_ups_all_or_nothing.md`); now every guest is `onboot=0` so the recovery script is the only starter. Deployed to `/etc/apcupsd/` on PVE via **atomic `mv`** (scp `.new` → `bash -n` → `mv`) with SAFETY_MODE left untouched (orchestrator stayed armed — verified `SAFETY_MODE` absent throughout). Verified: both live arrays show `111:lxc`, recover batch shows `111:Robot`, `pct config 111` → `onboot: 0`. No `recover.conf` / DB / systemd changes (guest-list edit is picked up by apccontrol at trigger time). **Dashboard:** the Shutdown Propagation card's curated display list `_UPS_DEVICES` in `js/health.js` (separate from the PVE scripts) also got `{kind:'lxc', id:111, label:'Robot'}` appended after Email + cache-bust `health.js?v=72→73` — static-file only, no `server.js`/pm2 change (the per-row live state comes from a dynamic `pct list` in the `_ups_inventory` handler, which already returns 111).
