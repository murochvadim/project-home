...
| [scripts/doshutdown](../scripts/doshutdown) → `/etc/apcupsd/doshutdown` (PVE) | The orchestrator. SAFETY_MODE-gated. **Phased shutdown** (added 2026-04-30): [1] QNAP halt via CGI → [2] **Phase A clients sequential, 60 s each** (LXCs 100, 103, 104, 105, 106, 108, 109, 110 + VM 101, halted one-by-one in numerical order — NOT parallel; the code blocks per guest) — these all write to Postgres / MQTT, must finish first → wait → [3] **Phase B services sequential, 30 s each** (LXCs 102 Postgres + 107 MQTT) — safe to halt now, no clients writing → wait → [4] write Phase 4 marker `/etc/apcupsd/last_shutdown_reason` → `shutdown -h now`. The phasing prevents transactions getting rolled back when Postgres/MQTT die mid-write — each client gets time to flush before its broker dies. |
