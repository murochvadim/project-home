...
- **⚠ OTA IS BROKEN on this board until v8 is USB-flashed (proven 2026-07-21).** Because it's **dual-homed on the SAME `192.168.1.0/24`** (WiFi `.167` + W5500 ETH `.139`), ESP-IDF makes ETH the **default netif** after ETH's DHCP, so replies to **inbound** WiFi connections (ArduinoOTA :3232, WebServer :80) egress via ETH toward EK15 and never return. The board MQTTs *out* fine (looks healthy) but nothing connects *in*. **Verified, not guessed:** espota "No response" from BOTH the Windows host AND LXC 104 (no NetBird), board fired zero OTA events, `curl http://.167/` → `HTTP 000`. NetBird was ruled out 3 ways (route on-link via WiFi; LXC-104 test; `home lan` route Not Selected). **v8 fix = `preferWifiDefaultRoute()`** (Kazir_15.ino): `esp_netif_set_default_netif(WIFI_STA_DEF)` after setup + re-asserted in the `ETH_GOT_IP` handler → inbound WiFi replies egress WiFi; the EK15 scan is unaffected (esp_ping pins to ETH via `cfg.interface`, resolveMac reads ETH's ARP cache directly). **Must be flashed ONCE via USB** (OTA can't reach the board to deliver its own fix — chicken/egg); after that, OTA works remotely. Built `.bin`: `C:\Users\muroc\AppData\Local\Temp\kz15_v8\Kazir_15.ino.bin`. ⚠ Not hardware-tested — verify OTA works right after the USB flash.
