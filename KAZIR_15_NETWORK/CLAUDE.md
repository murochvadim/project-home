...
- **Board = esp_boards subsystem** (`id='kazir_15'`, Project Boards tab, OTA, schema, `scan_now`/`eth_info` actions, `Network Monitor` action-group card in `esp-boards.js`). No BLE → OTA works normally. Params (runtime-tunable via `/config`, EEPROM-persisted): `scan_interval_sec` (300) / `ping_timeout_ms` (**800** since v12 — was 300, too short for the EK15 mesh round-trip so single sweeps caught only ~55% of reachable devices; 800 catches ~all, **verified 21→39 up/sweep 2026-07-21**) / `hosts_per_tick` (**8** since v12, raised from 4 so the longer timeout doesn't bloat the sweep).
...
- **Page features:** **device list** (online + offline in ONE list) with **15-min online grace** (a device seen in the last 15 min shows "connected" even if it missed the latest sweep — masks Deco power-save flicker, same idea as the home scanner's `ONLINE_GRACE_MIN`); **offline devices render red "disconnected"** with name + IP + **last-seen** (same look as Project Network); a **"Show offline only"** checkbox filters to the down ones; the list is **sorted by numeric IP ascending** (`.1 → .254`; client-side `ipNum` in `renderHosts`, so `.2` sorts before `.10` before `.100` — a text sort put `.102` before `.2`). **Per-MAC device naming** (inline edit, saved by MAC so it survives IP change / prune; `_kzEditing` gates the auto-refresh so typing isn't clobbered); **Scan Now** (fires the board's `scan_now`); **category summary card** — instead of raw "Hosts Up/Seen" it shows **Cameras / Access Points / Deco on-and-off tiles** (`GET /api/kazir15/summary`: `bool_or(up)` grouped by MAC, classified from the device name via regex `/cam/`, `/\bap\b|unifi/`, `/deco/`), plus the board's eth link/ip/gw.
...
- **Scan coverage was FIXED (v12, 2026-07-21) by raising `ping_timeout_ms` 300→800** — the misses were NOT power-save (the devices are mains-powered cameras/APs); the 300 ms timeout was just shorter than the EK15 mesh round-trip. Verified empirically: single-sweep `hosts_up` jumped 21→39 (≈ the full reachable set = the 15-min-grace count). `hosts_per_tick` raised 4→8 to keep the sweep ~fast despite longer timeouts (empty IPs dominate the sweep time via their timeout). The loop-task WDT is disabled, so a *future* firmware hang wouldn't self-recover.
