...
**⚠ Gotcha — the Windows host must NOT accept the `home-lan` route (fixed 2026-06-08).** The dashboard host (`newasus`, WiFi, currently `192.168.1.182` — DHCP, was `.128`) is itself **physically on `192.168.1.0/24`**, but it was also a NetBird client that **accepted** the `home-lan` route. That route (`wt0`, total metric ~6) beat the directly-connected WiFi route (~296), so the host tunnelled its OWN local LAN: every connection to DB/MQTT/HA/boards hairpinned host → `wt0` → LXC 108 → NAT'd to `192.168.1.195` → back onto the LAN. It "worked" via that SNAT but was fragile (NetBird down = host loses the whole LAN) and **broke dashboard OTA** — espota's board→host connect-back failed ("No response from device") because the host advertised the unreachable overlay IP and its return path was asymmetric. **Fix:** on the host, `netbird routes deselect "home lan"` (the route ID is literally `home lan`; persists; reverse with `netbird routes select "home lan"`). After deselect the host reaches the whole LAN WiFi-direct (`src 192.168.1.182`); dashboard→DB verified working; NetBird stays Connected (2/2 peers) — remote LAN access is via the `netbird-gw` peer, NOT the host, so deselecting on the host doesn't affect remote access. A peer that lives on a routed network should always deselect that network's route. See [[incident_netbird_breaks_ota]].
