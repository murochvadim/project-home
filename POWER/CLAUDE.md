...
> **Implemented 2026-05-29 as a separate Settings tab** rather than the 4-sub-card Card-6 originally specced. The Settings tab carries: Billing Cycle / Tariff (with IEC website link) / Past Bills cards, plus a **Power Outage Log** card (added 2026-06-08). The current period readout (kWh + cost + days elapsed) lives on the Status tab's LCD Total card so it's always visible.
>
> **Power Outage Log card** (Settings tab, 2026-06-08) — permanent mains-loss history from the UPS: table of Started · Ended · Duration · Status, newest first; ongoing outages show a live elapsed timer + red `ON BATTERY` badge. Reads `GET /api/power/outages` (own module `routes-power-outages.js`, NOT inline in server.js — the `/api/power/*` routes are inline and a new `app.get(` there would trip the architecture-guard hook). Backed by the forever-retained `ups_power_events` table on LXC 102, filled by the 60-s `ups_poll.py` reconcile from the apcupsd `ups_onbattery` alerts (no apcupsd hook edits). Full design in [UPS/CLAUDE.md](../UPS/CLAUDE.md) → `ups_power_events` DB-schema sub-section. `js/power.js` `loadPowerOutages()`, loaded on Settings tab-show + refreshed every 5 s while Settings is visible. Top-consumers + history-reconciliation are P4+ work — see Phase rollout for the full P7 description. **The P3 Discovery Rule's knobs were intentionally NOT put on this tab** — they ship as sentence-driven `r_power_discovery_init` together with the P3 rule via `/create-rule`, keeping every rule's tunables in one place (Main Agent → Base Rule Settings). Detail below is the original design for reference.
...
| **P2 — Device Registry** (✓ done 2026-05-29) | unified `+ Add Device` form on the Project Power page → device picker (existing devices OR "Other custom" for unmanaged loads) + optional display-name override + Phase (R / S / T / R.S.T) + Room + Power. Behavior radio at the end: **always_on** (constant baseline) or **auto** (rule detects on/off from Shelly deltas). Single-phase rows take Expected + optional Max; R.S.T rows take per-phase Expected + Max so variable-power devices (dishwasher's heating peak) are recognized across their full range. Two new columns: **Live W** (red wattage when rule says ON, grey OFF default, always_on rows always render as ON) + **Last seen** (relative timestamp of the last state change). All rows editable; cyclic dropped as a behavior — devices that cycle just register as auto and the rule handles the cycling. **Channel-aware (since 2026-05-29 evening)**: multi-gang switches like Kitchen Switch expand to one dropdown entry per channel (`Kitchen Switch – High Light` / `… – Frig Spots` / `… – Spare`), so the user registers each load with its own wattage signature, not the parent switch. Backed by new columns `power_devices.channel TEXT NULL` + synthetic `power_devices.row_id BIGSERIAL PRIMARY KEY` (previous PK on device_id dropped — multiple rows can share device_id now). UNIQUE constraint on `(device_id, COALESCE(channel, '__nochan__'))` so the same (device, channel) pair can't be double-registered. All CRUD endpoints rekeyed on `row_id` (`PATCH /api/power/devices/:row_id` / `DELETE /api/power/devices/:row_id` / reorder POST takes `row_id` list). The available-devices endpoint expands `channel_config` and skips channels marked `enabled: false`; channels without a `name` fall back to `Ch <key>`. Single-channel devices (microwave, fridge, AC) stay as single rows (channel=NULL). Custom rows (manual_*/auto_* virtuals) are never channel-scoped. |
| **Known State behavior** (✓ done 2026-05-29 evening) | Third behavior option beside `always_on` + `auto`. For devices whose ON/OFF state is **directly reported** by HA (TVs / Alexas / soundbar / any media_player). New `power_devices.source = 'state_known'`. Live W shows `expected_w` when source's HA state ∈ ANY value except `{off, unavailable, unknown, standby, none, '', null}`, grey OFF otherwise. New rule `RULES/rules/power_known_state.py` (wildcard trigger with TTL-cached early-return on state_known device_id set) writes `power_devices.live` + emits `virtual:power_state` on every transition — same output path as P3 Discovery so the LCD DEVICES ON counter and history reproduce identically. POST endpoint seeds `live` from the device's current `last_state.state` at registration time so the LCD is accurate immediately without waiting for the next state event. **Wired into `DEVICE/agent/adapters/ha_api.py` HA_DIRECT_DEVICES** (2026-05-29 evening) — added 3 Samsung TVs (`media_player.samsung_85_qled` / `samsung_q60ba_50_tv_2` / `samsung_q49_ba_tv`) + 5 Alexas (`10inch_echo_show` / `alexa_guy_room` / `samsung_soundbar_2` / `alexa_maya_bedroom` / `alexa_my_bathroom`), each projecting their HA state into `devices.last_state.state`. New `devices` rows inserted for the 3 TVs (`device_type='tv'`, `protocol='ha_api'`, `dps_labels={"state":"State"}`); the 5 Alexas already existed as identity-only rows and just needed bridge wiring. Discovery rule filter `source IN ('auto_pending','auto_custom','auto_discovered')` automatically excludes `state_known` rows from Shelly-Δ inference. state_known requires the linked path (custom virtual rows have no real device.state to read). Validation in POST/PATCH refuses `state_known` for virtual `manual_*`/`auto_*` rows. **Device-agent defensive fix (same evening, exposed by restart):** `device_agent._load_devices` was crashing when iterating `dps_config` of legacy `manual_*` virtual rows whose values are flat scalars (`{"phase":"R","nominal_w":200}`) instead of per-channel dicts; added `isinstance(cfg, dict)` guard. |
| **P3 — Discovery Rule** (✓ done 2026-05-29, channel-aware update same day) | `power_phase_discovery.py` on LXC 105 (rule group `power`, priority 20, trigger `shelly_3em_main`, depends_on `Power Ingest`). Registry rows now carry `row_id` + `channel`; all per-device state (`candidates`, `lock_until`, `live_state`) keyed by `str(row_id)` so multiple channels of the same parent switch are independent. `virtual:power_state` dps gains `channel` field. **Pure state recognition** — does NOT learn signatures. On every Shelly event computes Δr/Δs/Δt vs in-memory `state.shared['_power_discovery.prev_w']`; if max|Δ| < noise_floor_w returns immediately. Otherwise loads `power_devices` registry (TTL-cached 30 s, filtered to `source IN ('auto_pending','auto_custom','auto_discovered')`) and for each device checks whether Δ on its phase falls inside `[expected_w × tol_low, max_w × tol_high]` AND sign matches desired transition (positive Δ when device is OFF, negative when ON). 3-phase devices require ALL three per-phase Δ to hit their per-phase range with matching sign simultaneously. Candidates must persist past `settling_sec` before flipping; on confirmed flip writes `power_devices.live = {on, w (or r_w/s_w/t_w), ts}` + `last_observed_at=NOW()` AND emits `virtual:power_state` event `{device_id, phase, on, w}` for history queries. After each flip the device is locked for `post_flip_lock_sec` (default 5 s) — filters Shelly's per-phase oscillation on sharp on/off edges. Multi-candidate tiebreaker: lowest `\|Δ-expected_w\|/expected_w` score wins per tick. **Knobs sentence-driven** via `r_power_discovery_init` container (5 sentences, defaults: noise floor 10 W, settling 2 s, tol_low 0.7, tol_high 1.15, post-flip lock 5 s); KNOB_PATTERNS in rule_engine.py extended with new `watt` and `ratio` target units (previously only `sec`/`min`/`count`). End state: Live W + Last seen columns light up in the registry as the engine sees transitions; LCD's DEVICES ON counter grows during heavy load. **⚠ Windowed-plateau fix (2026-06-08):** the original code re-baselined `prev_w` on EVERY Shelly event (~1/sec, often several/sec), so for a load that **ramps** (e.g. an inverter AC over ~10 s) every event-to-event Δ was far below threshold → auto devices were **NEVER recognized** (live stuck off; the living-room AC showed OFF while running). Root cause confirmed: at the raw 1 s event cadence no single Δ reached the AC's ~824 W phase-R floor, yet the 13 s ingest snapshots caught a clean full-3-phase step (R+1145/S+630/T+787). Fix: P3 now measures each phase against the sample from **~`delta_window_sec` ago** (default **20 s**, new knob `power_discovery.delta_window_sec`) via a **trailing sliding window** over a small in-memory history (`_power_discovery.history`, pruned to ~2 windows), so a gradual ramp accumulates into one big Δ **regardless of when it started**. **Window widened 12 → 20 s (2026-06-08):** the ACs are real 3-phase loads but their phases switch a few seconds apart (observed: T drops ~15 s BEFORE R/S at shutdown), and a 12 s window couldn't span the staggered transition so the all-phases-must-match check failed on OFF — 20 s spans it. (⚠ Don't "fix" a staggered 3-phase device by zeroing a phase's `expected_w` in `power_devices.config` — that silently changes the user's registration + removes the disambiguator between similar devices; widen the window instead.) (A first iteration used a fixed-tick "plateau" that re-anchored on the every-tick `if not cands` path — that could re-anchor mid-ramp and straddle/miss the step, so it was replaced with the sliding window before the user's final audit.) Caveats: detection is transition-based with ~1-window latency; two devices switching within the same window can briefly mask each other (resolves as the first step ages out); and a device already ON when the engine (re)starts isn't flagged until its next OFF→ON cycle (engine `live_state` only learns from transitions). |
