...
`GET /chats` · `GET /groups` · `GET /contacts` · `GET /messages?jid=&limit=` (chat pane + per-`sender_jid`
grouping) · `POST /history {jid,count}` (older history on demand).
**Write (through the send-guard):** `POST /send {jid,text,force?}` · `POST /leave {jid}` (groups; outward +
irreversible) · `POST /delete {jid,key}` · `POST /read {jid}` · `POST /relink`.
...
(`data_table/settings_table NULL`, like email — a service cache, not a decision loop). Register in Health
DB-Volumes + retention_policies (P2 doc step).
