...
| Smart switch (TS0044 wireless scene remote) | **LIVE** — the real TS0044 `0x8c65a3fffeb139ae` (Z2M friendly name still `Balcony Smart Switch`), **moved here from the Balcony agent 2026-07-26** |
...
- **Smart Switch** — Balcony-equivalent wallmote-style binding UI for the TS0044 scene remote (single-press only, hold doesn't fire on this firmware variant). **Live since 2026-07-26** — the real TS0044 (`0x8c65a3fffeb139ae`) was **moved here from the Balcony agent**; started with **empty** bindings (re-bind per button). The Balcony agent's Smart Switch tab + handler were deleted. **Scene bindings (2026-07-29):** the picker has a **★ Scenes** section (mirrors the Living Room Wallmote) — tick a Main Agent scene (`apartment.scenes`) to store `{type:'scene', target}`; the handler emits `{action:'run_scene', scene}` and the engine expands + dispatches it. The button card shows `🎬 <Scene>`. Save serializer preserves `type`/`target`; `bcBindingTag` has a `scene` case. **Double-click / hold are NOT exposed** — live testing (2026-07-29) confirmed this MOES firmware emits `_double` only ~1 in 6 fast taps and **never `_hold`** (0 in 90 days + 0 live), so single-press stays the only reliable gesture.
- **HASP-relay `toggle` fix (2026-07-26):** binding a button to a HASP on-board relay (e.g. `hasp:mybathroom-panel:p1b20` = the main light, `dps_config.relay.output='output2'`) with **action `toggle`** used to only ever turn ON — the button device is **stateless** (`last_state=NULL`), so the toggle resolver read nothing and defaulted to turn_on every press. Fixed: `_hasp_relay_state()` (in `my_bathroom_smart_switch_handler.py`, mirrored in `balcony_buttons.py` + `panel_commands.py`) reads the relay's live on/off from the **plate device's `last_state->>output<n>`** — the authoritative, always-fresh source (`rule_engine._update_hasp_panel_state` rewrites `last_state` on every relay change; in-memory `state.devices` is stale for output keys). The output mapping + state are read in **one DB query** so it never depends on in-memory `dps_config`. Result: a real one-button on/off toggle, **no drift** (reads true state, which stays current through motion/manual/auto-off) — no 2-button workaround needed. See [[project_hasp_relay_toggle]].
...
| My BathRoom Smart Switch Handler | [`my_bathroom_smart_switch_handler.py`](../RULES/rules/my_bathroom_smart_switch_handler.py) | **`0x8c65a3fffeb139ae`** (real TS0044, moved from balcony 2026-07-26; Z2M friendly name still `Balcony Smart Switch` — the rule keys off the stable IEEE id, so the display name doesn't matter) | TS0044 button event → device commands per `dashboard_settings.my-bathroom.smart_switch_bindings`. Also supports **scene bindings** (`{type:'scene',target}` → `run_scene`, 2026-07-29) — scene branch is FIRST in `_build_commands`, before the `device_id` check, since a scene binding carries `device_id='scene:<Name>'` |
