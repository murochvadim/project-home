...
- **Smart Switch** — Balcony-equivalent wallmote-style binding UI for the TS0044 scene remote (single-press only, hold doesn't fire on this firmware variant). **Live since 2026-07-26** — the real TS0044 (`0x8c65a3fffeb139ae`) was **moved here from the Balcony agent**; started with **empty** bindings (re-bind per button). The Balcony agent's Smart Switch tab + handler were deleted.
...
| My BathRoom Smart Switch Handler | [`my_bathroom_smart_switch_handler.py`](../RULES/rules/my_bathroom_smart_switch_handler.py) | **`0x8c65a3fffeb139ae`** (real TS0044, moved from balcony 2026-07-26; Z2M friendly name still `Balcony Smart Switch` — the rule keys off the stable IEEE id, so the display name doesn't matter) | TS0044 button event → device commands per `dashboard_settings.my-bathroom.smart_switch_bindings` |
