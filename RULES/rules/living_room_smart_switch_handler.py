"""Living Room Smart Switch Handler — dispatch device commands on button presses.

Reads bindings from `dashboard_settings` (key=`living-room.smart_switch_bindings`),
saved by the Living Room Agent dashboard page. The TS0044 publishes a single
...
no deferred dispatch. Clone of `my_bathroom_smart_switch_handler.py`.
...
log = logging.getLogger("rule.living_room_smart_switch_handler")

# Tuya TS0044 4-button remote (Zigbee IEEE id). Paired 2026-08-12 for the Living
# Room (Z2M friendly name "Living Room Smart Switch"; the id is the stable IEEE so
# the rule fires regardless of the display name).
SMART_SWITCH_ID = "0x94b216fffeb32c65"

RULE = {
    "name":        "Living Room Smart Switch Handler",
    "description": "When you press the Living Room wireless wall remote, this runs whatever each button is set to do. (Only a single press works on this remote.)",
...
def _get_bindings(state):
    ...
    rows = state.db_query(
        "SELECT value FROM dashboard_settings WHERE key = 'living-room.smart_switch_bindings'"
    )
    data = {}
    if rows:
        v = rows[0][0]
...
def _resolve_target(action_str, device_id, state, channel):
    ...
    if action_str != "toggle":
        return None
    # HASP on-board relay: read the real on/off from the plate, not this stateless
    # button, so toggle alternates.
...
def _build_commands(slot, state, slot_key):
    ...
    for b in slot:
        if b.get("type") == "scene":
            ...
            # Must come BEFORE the device_id check since a scene binding carries
            # device_id='scene:<Name>' (truthy).
            tgt = b.get("target")
            if tgt:
                commands.append({"action": "run_scene", "scene": tgt,
                                 "rule": "Living Room Smart Switch Handler"})
            else:
        ...
        special_cmd = (_build_alexa_cmd(b, action, device_id, "Living Room Smart Switch Handler", dev)
                       or _build_vacuum_cmd(b, action, device_id, "Living Room Smart Switch Handler", dev))
        if special_cmd:
...
def evaluate(event, state):
    ...
    log.info("Living Room Smart Switch %s → %d commands: %s",
             slot_key, len(commands),
             [(c.get("device_id"), c.get("channel"), c.get("action")) for c in commands])
    return commands
