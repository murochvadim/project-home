"""My BathRoom Buttons — dispatch device commands on HASP panel button presses.
...
`hasp/my-bathroom/state/p<page>b<id>` MQTT messages — see
`rule_engine.on_mqtt_event()` (the "Button widgets" branch). The synthetic
event has `device_id = 'hasp:my-bathroom:p<page>b<id>'` and dps =
...
RULE = {
    ...
    "depends_on": [],
    "test_event": {
        "device_id": "hasp:my-bathroom:p1b110",
        "source": "hasp_button",
        "dps": {"event": "short"},
    },
}
...
_BTN_RE = re.compile(r"^hasp:my-bathroom:p(\d+)b(\d+)$")
...
def _build_command(b, state):
    ...
    log.warning("My BathRoom Buttons: unsupported binding type '%s'", btype)
    return None
...
def evaluate(event, state):
    ...
    if not bindings:
        log.info("My BathRoom Buttons: no binding for p%db%d:%s", page, bid, evt)
        return []
    ...
    if commands:
        log.info("My BathRoom Buttons: p%db%d:%s → %d command(s)",
