"""Balcony Button Mirror — keep panel button visuals synced to real device state.

The complement of `balcony_buttons.py`:
  buttons.py:  panel press → device command   (input: hasp → device)
  mirror.py:   device state → panel pXbY.val  (output: device → hasp)

For every `hasp_buttons` row with action_type='device' on the balcony panel,
this rule subscribes (via wildcard early-return) to the bound device's state
events and publishes `hasp/balcony/command/p<page>b<button_id>.val` = 0/1
...
log = logging.getLogger("rule.balcony_button_mirror")
...
def evaluate(event, state):
    ...
    for b in bindings:
        ...
        commands.append({
            ...
            "value": target_val,
            "rule": "Balcony Button Mirror",
            "_skip_loop_guard": True,
        })

    if commands:
        log.info("Balcony Button Mirror: %s → %d button visuals updated",
                 dev_id, len(commands))
    return commands
